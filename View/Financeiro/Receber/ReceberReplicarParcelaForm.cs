...
namespace Dados.View.Financeiro.ReceberReplicarParcelaFormulario
{
    public partial class ReceberReplicarParcelaForm : Form
    {
        ...
        public FinanceiroReceber ParcelaBase { get; set; }

        public BindingList<FinanceiroReceber> ParcelasGeradas { get; private set; } = new();

        public ReceberReplicarParcelaForm(DataContext context)
        {
            InitializeComponent();
        ...
        private void gridConteudo_CellEndEdit(object sender, DataGridViewCellEventArgs e)
        {
            ...
            {
                if (gridConteudo.Rows[e.RowIndex].DataBoundItem is FinanceiroReceber parcela)
                {
        ...
        private void ReceberReplicarParcelaForm_Load(object sender, EventArgs e)
        {
            if (ParcelaBase == null) 
                return;
        ...
        private void GerarParcelas()
        {
            ...
            for (int i = 1; i < qtde; i++)
            {
                ...
                var nova = new FinanceiroReceber
                {
        ...
        private void ReceberReplicarParcelaForm_KeyDown(object sender, KeyEventArgs e)
        {
            switch (e.KeyCode)
            {
                case Keys.Escape:
